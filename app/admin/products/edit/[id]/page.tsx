...
export default function EditProductPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { products, initialLoading } = useAdmin();
  ...
  useEffect(() => {
    ...
    if (product) {
      ...
      setInrPrice(product.inrPrice !== undefined && product.inrPrice !== null ? product.inrPrice.toString() : '');
      setSalePrice(product.salePrice !== undefined && product.salePrice !== null ? product.salePrice.toString() : '');
      setInrSalePrice(product.inrSalePrice !== undefined && product.inrSalePrice !== null ? product.inrSalePrice.toString() : '');
      setStockStatus(product.stockStatus || 'in_stock');
      setHasVersions(product.hasVersions || false);
      if (product.hasVersions && product.versions && product.versions.length > 0) {
        setVersions(product.versions.map((v: any) => ({
          ...v,
          ...
          inrPrice: v.inrPrice !== undefined && v.inrPrice !== null ? v.inrPrice.toString() : '',
          inrSalePrice: v.inrSalePrice !== undefined && v.inrSalePrice !== null ? v.inrSalePrice.toString() : '',
          isSubscription: v.isSubscription || false,
          billingPeriod: v.billingPeriod || 'monthly',
          planId: v.planId || null
        })));
      }
      setCategory(product.category || 'Plugin');
      setTags(product.tags ? product.tags.join(', ') : '');
      setRequiresLicense(product.requiresLicense ?? true);
      setIsSubscription(product.isSubscription || false);
      setBillingPeriod(product.billingPeriod || 'monthly');
      setPlanId(product.planId || null);
      setInitialized(true);
    }
  }, [products, initialLoading, resolvedParams.id, initialized]);
  ...
  const handlePublish = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!hasVersions && isSubscription && (!currentPlanId || currentPlanId === 'pending_new')) {
        // Create Razorpay Plan if converting to subscription OR regenerating (single product)
      ...
      if (hasVersions) {
        for (let i = 0; i < versions.length; i++) {
          let v = versions[i];
          if (v.isSubscription && (!v.planId || v.planId === 'pending_new')) {
        ...
        setVersions([...versions]); // Trigger re-render to save new planIds in state just in case
      }

      await updateDoc(doc(db, "products", resolvedParams.id), {
        name,
        description,
        ...
        requiresLicense,
        isSubscription: hasVersions ? false : isSubscription, // if hasVersions, the product itself is not the subscription
        billingPeriod: (hasVersions || !isSubscription) ? null : billingPeriod,
        planId: (hasVersions || !isSubscription) ? null : currentPlanId,
      });
  ...
  const productUrl = typeof window !== 'undefined' ? `${window.location.origin}/products/${resolvedParams.id}` : `https://crevostore.com/products/${resolvedParams.id}`;

  if (initialLoading || !initialized) {
    return <div style={{ padding: '64px', textAlign: 'center', color: textMuted }}>Loading product studio...</div>;
  }
    ...
    <div className={styles.studioContainer}>
      ...
      <div className={styles.studioHeader}>
        <div className={styles.titleArea}>
          ...
          <h1 className={styles.pageTitle}>{name || 'Untitled Product'}</h1>
        </div>
        ...
        <div className={styles.formColumn}>
                ...
                {!hasVersions ? (
                      ...
                      <div style={{ background: 'rgba(0, 113, 227, 0.05)', padding: '16px', borderRadius: '14px', border: '1px solid rgba(0, 113, 227, 0.2)', display: 'flex', flexDirection: 'column', gap: '12px' }}>
                        ...
                        <div className={styles.formGroup}>
                          ...
                          <select 
                            ...
                            disabled={!!planId && planId !== 'pending_new'}
                          >
                        ...
                        {planId && planId !== 'pending_new' ? (
                          <div style={{ fontSize: '0.75rem', color: 'var(--success)' }}>
                            Active Razorpay Plan: <code>{planId}</code>
                            <button type="button" onClick={() => setPlanId('pending_new')} className={styles.btnSecondary} style={{ marginLeft: '12px', padding: '4px 10px', fontSize: '0.7rem' }}>Regenerate Plan</button>
                          </div>
                        ) : (
                          <div style={{ fontSize: '0.75rem', color: textMuted }}>Razorpay billing plan will automatically be created on save.</div>
                        )}
                      </div>
                    )}
                  </>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                    {versions.map((version, idx) => (
                      <div key={version.id} style={{ border: `1px solid ${borderColor}`, borderRadius: '16px', padding: '20px', background: 'var(--bg-secondary)', display: 'flex', flexDirection: 'column', gap: '16px' }}>
          ...
          {activeTab === 'share' && (
            <div className={styles.sectionCard}>
              ...
              <p className={styles.cardSubtitle}>Copy direct checkout links or share your digital asset to social media.</p>
              ...
              <div className={styles.shareGrid}>
                ...
                >
                  <span>🔗</span>
                  <span>{copiedLink ? 'Copied to Clipboard! ✓' : 'Copy Product URL'}</span>
                </button>

                <a 
                  href={`https://twitter.com/intent/tweet?text=${encodeURIComponent(`Check out ${name} on Crevo Store!`)}&url=${encodeURIComponent(productUrl)}`}
                  target="_blank"
                  rel="noreferrer"
                  className={styles.shareBtn}
        ...
        <div className={styles.previewColumn}>
          ...
          <div className={styles.deviceFrame}>
            <div className={styles.deviceTopBar}>
              ...
              <div className={styles.deviceUrl}>crevostore.com/products/...</div>
            </div>
